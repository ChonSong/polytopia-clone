...
## Phase 5 — Multiplayer Foundation (2026-07-01)

**Generated by Coach backlog refresh.** Target: enable 2+ human players to play a full game over the network. Reuses existing Next.js frontend and Python FastAPI backend.

### Task: add-websocket-game-server
**Priority:** P1
**Description:** Add WebSocket support to the existing FastAPI backend (or standalone Node.js server) to handle real-time game communication. The Phaser WebGL client already supports WebSocket natively. Must handle:
1. **Game room lifecycle** — create room, join room by ID, leave room, host disconnect handling
2. **Connection management** — WebSocket upgrade endpoint, heartbeat/ping-pong every 15s, graceful disconnect cleanup (timeout stale connections after 30s)
3. **Message protocol** — typed JSON messages with action field (JOIN, LEAVE, READY, TURN_ACTION, STATE_SYNC, CHAT). All messages validated server-side.
4. **State authority** — server is authoritative for GameState. Clients propose actions via TURN_ACTION, server validates and broadcasts updated STATE_SYNC.
5. **Error handling** — malformed message → disconnect with reason, reconnection token for brief drops

**Success criteria:**
- Client connects to WebSocket server from browser (wss:// or ws://)
- Create room returns room code; second client joins by code
- Server broadcasts JOIN/LEAVE events to all room members
- Disconnecting client is detected within 30s, room updated
- No regression in existing 523 passing tests
- 0 console WebSocket errors on connect/disconnect

### Task: add-game-state-delta-sync
**Priority:** P1
**Description:** GameState serialization for multiplayer. The existing GameState is turn-based and single-player. Add delta sync so only changes (not full state) are sent each turn. Handle:
1. **Delta computation** — compute diff between current and previous GameState, send only changed fields (units moved, cities updated, techs researched, scores changed)
2. **Full state sync** — on reconnect or initial join, send complete GameState snapshot
3. **Map/Set serialization** — existing SaveManager already handles Map/Set for localStorage. Reuse that logic for WebSocket serialization.
4. **Turn order enforcement** — server tracks whose turn it is, rejects out-of-turn actions
5. **Latency tolerance** — 5s grace period for turn actions before auto-pass (configurable)

**Success criteria:**
- Two clients see identical GameState after each turn (SSOT verification)
- Delta messages are smaller than full state (measured: delta < 30% of full sync size)
- Reconnecting client receives full sync and resumes correct state
- Server rejects actions from wrong player
- No regression in existing 523 passing tests

### Task: add-lobby-system
**Priority:** P2
**Description:** Pre-game lobby for creating and joining multiplayer games. Accessible from SelectScene:
1. **Create Game** — host selects map size, game speed, difficulty, and tribe. Generates 6-character room code displayed prominently.
2. **Join Game** — enter room code to join. Shows host's settings and player list (slots 2-8 players).
3. **Ready state** — each player toggles READY. Host starts game when all ready. Ready lock prevents late setting changes.
4. **Public game browser** — optional: list public rooms with player count, map size, status (waiting/in-progress).
5. **Cancellation** — host can cancel lobby (disconnects all). Player can leave lobby (returns to tribe selection).

**Success criteria:**
- Create Game shows room code; Join Game connects by code
- Player list updates in real-time as players join/leave
- All players ready → game starts with correct settings
- Cancel/Leave returns to SelectScene without errors
- No regression in existing 523 passing tests

### Task: add-multiplayer-ui-in-game
**Priority:** P3
**Description:** In-game multiplayer UI elements:
1. **Player panel** — list of all players with tribe icon, turn indicator (active/waiting/disconnected), connection status (green/yellow/red)
2. **Turn timer** — countdown bar showing remaining time for current player (server-enforced)
3. **Disconnect overlay** — if opponent disconnects, show "Waiting for [player] to reconnect..." with timeout countdown
4. **Chat** — simple text chat sidebar (Enter to send, scrollable history, system messages for join/leave/ready)
5. **Connection indicator** — persistent icon in HUD showing WebSocket status (connected/reconnecting/disconnected)

**Success criteria:**
- Player panel shows all room members with correct tribe icons
- Turn countdown decrements in real-time
- Disconnect overlay appears on opponent disconnect, clears on reconnect
- Chat messages appear for all connected clients
- No regression in existing 523 passing tests

### Task: add-multiplayer-turn-management
**Priority:** P2
**Description:** Server-authoritative turn management for multiplayer games:
1. **Turn queue** — server maintains ordered list of active players. After current player submits END_TURN, server advances to next player.
2. **Action validation** — server validates each TURN_ACTION against current GameState before broadcasting (unit exists at position, has enough moves, target is valid).
3. **Concurrent safety** — one action at a time per player. Queue out-of-order actions and replay in correct sequence.
4. **Timeout handling** — if player doesn't submit action within time limit, server auto-passes their turn (skips to next player, no auto-move).
5. **Game end** — server detects elimination/victory conditions and broadcasts GAME_OVER with final scores.

**Success criteria:**
- Game progresses turn-by-turn with all players seeing same sequence
- Invalid actions are rejected with error message
- Idle player is auto-skipped after timeout
- Game ends correctly when only one player remains
- No regression in existing 523 passing tests
